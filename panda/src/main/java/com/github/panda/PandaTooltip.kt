...
fun PandaTooltip(
    ...
    pandaState: Boolean = false,
    backgroundColor: Color,
    requesterView: @Composable (Modifier) -> Unit,
    tooltipContent: @Composable () -> Unit,
    pandaDismissRequest: ((Boolean) -> Unit),
) {
    var isShowTooltip by remember { mutableStateOf(pandaState) }
    ...
    if (isShowTooltip) {
        PandaTooltip(
            onDismissRequest = {
                pandaDismissRequest(isShowTooltip.not())
            },
            position = position,
...
fun PandaTooltip(
    ...
    val tooltipPositionProvider = remember(alignment, offset) {
